...
def tsne_(X, Y):
    ...
    target_ids = range(len(set(Y)))  # Y is labels
    plt.figure(figsize=(6, 5))
    colors = plt.cm.get_cmap('hsv', len(target_ids))
    for i, label in zip(target_ids, target_ids):
        plt.scatter(X_2d[Y == i, 0], X_2d[Y == i, 1], c=np.random.rand(3, ), s=3, label=label)
    plt.legend()
    plt.show()
...
def initFaceDetector():
    minsize = 20
    caffe_model_path = "/home/cmti_face/workspace/asian_face_clean/mtcnn/model"
...
def main():
    ...
    gpu_id = 0
    face_recon = FaceRecogniton(gpu_id)  # Init the face recognition class

    # error = []
    f = open(imglistfile, 'r')
    root = "/home/cmti_face/workspace/asian_face_clean/face_set_2/face_set_2"
    ...
    for dir in dirs:
        folder = root + "/" + dir + "/" + "*.jpg"
        imagesdir = glob.glob(folder)

        embeddings = []
        for impath in imagesdir:
            ...
            tmp = img_matlab[:, :, 2].copy()
            img_matlab[:, :, 2] = img_matlab[:, :, 0]
            img_matlab[:, :, 0] = tmp  # change from BGR to RGB
            ...
            smallsize  = min(height, width )
            bigsize = max(height, width)
            retx1 = centerx - int(bigsize / 2)
            retx2 = centerx + int(bigsize / 2)
            rety1 = centery - int(bigsize / 2)
            rety2 = centery + int(bigsize / 2)

            # remove if  retx1, x2, y1, y2 is out side of boundary
            # if retx1 < 0 or retx2 > w or rety1 < 0 or rety2 > h:
            #     if isremove:
            #         os.remove(impath)
            #     continue
           ...
           # remove small face
            ...
            ##
            #flag, feat = face_recon.get_fetures(img)  # extract features
            ...
            for i in range(len(points)):
                for j in range(0, 5, 1):
                    print(int(points[i][j]), int(points[i][j + 1]))
                    cv2.circle(img, (int(points[i][j]), int(points[i][j + 5])), 2, (0, 0, 255))
        ...
        # tsne_(embeddings, 1)
        imagesdirs = glob.glob(folder)
        if len(imagesdirs) < 10:
            shutil.rmtree(root + "/" + dir, ignore_errors=True)
